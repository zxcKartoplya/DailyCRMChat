...
async function quickDelete(report: DailyReport) {
	if (
		!confirm(`Удалить дейлик за ${report.report_date} и связанное сообщение?`)
	)
		return
	try {
...
const canSend = computed(
	() => messageText.value.trim().length > 0 && !chatStore.sending,
)
...
async function send() {
	...
	const payload = {
		...
		daily_report: showReportForm.value
			? { ...reportPayload.value, source: 'internal_web' as const }
			: null,
	}
...
<template>
	<div class="h-screen bg-slate-900 flex overflow-hidden">
		<!-- ── LEFT SIDEBAR ─────────────────────────────────────────────────── -->
		<aside
			class="w-72 flex-shrink-0 bg-slate-800 border-r border-slate-700 flex flex-col"
		>
			<!-- User card -->
			<div class="px-4 py-4 border-b border-slate-700 h-[100vh]">
			...
			<div class="flex-1 flex flex-col overflow-hidden">
				...
				<div class="flex-1 overflow-y-auto px-2 pb-2 h-[100vh]">
		...
		<!-- ── MAIN CHAT AREA ──────────────────────────────────────────────── -->
		<main class="flex-1 flex flex-col min-w-0 h-[100vh]">
			<!-- Header -->
			<div class="px-5 py-3.5 border-b border-slate-700 flex-shrink-0">
				...
				<p class="text-xs text-slate-400">
					Отправь сообщение с дейликом или без него
				</p>
			</div>
			...
			<div ref="messagesEl" class="flex-1 overflow-y-auto px-4 py-4 space-y-1">
				<div v-if="chatStore.loading" class="flex justify-center py-10">
					<svg
						class="animate-spin h-6 w-6 text-slate-500"
						fill="none"
						viewBox="0 0 24 24"
					>
						<circle
				...
				<template v-else>
					<div
						...
						class="flex flex-col items-center justify-center h-full text-slate-500 gap-2 pt-16"
					>
					...
					<ChatMessage
						...
						@open-report="
							id =>
								(selectedReport =
									reportsStore.dailyReports.find(r => r.id === id) ?? null)
						"
					/>
				</template>
			</div>
			...
			<!-- Compose -->
			<div
				class="flex-shrink-0 border-t border-slate-700 bg-slate-800/60 backdrop-blur-sm"
			>
				<!-- Daily report form (collapsible) -->
				<Transition name="slide-down">
					<div
						v-if="showReportForm"
						class="px-4 pt-3 pb-1 border-b border-slate-700/50"
					>
						<DailyReportForm
				...
				<div class="px-4 py-3 flex items-end gap-2">
					...
					<!-- <button
						:title="
							showReportForm ? 'Скрыть форму дейлика' : 'Прикрепить дейлик'
						"
						:class="[
